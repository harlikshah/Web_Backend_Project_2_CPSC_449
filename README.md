...
Here it is used Reddit API to retrieve posts from Reddit. These posts are used to populate the DynamoDB posts table and Redis votes table key-value store.
...
#### -----------------Porting the posting microservice to Amazon DynamoDB Local----------------------
* Create a new post
```shell script
...
#### ---------------------Porting to the voting microservice to Redis---------------------------
1. Upvote a post
```shell script
...
#### ---------------------- Aggregating posts and votes with a BFF---------------------------
